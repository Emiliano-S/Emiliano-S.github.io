document.addEventListener("DOMContentLoaded", () => {
    ...
    function changeLanguage(languageKey) {
      ...
      updatePageContent(lang); // Update content immediately
      showPopup(); // Call popup separately
    ...
    function loadTranslation(language, callback) {
      fetch(`assets/i18n/${language}.json`)
        ...
        .catch((error) => {
          ...
          // Default or fallback content can be set here
    ...
    function updatePopupMessage(translation, minutes, name, city) {
      ...
      message = message.replace("{{minutes}}", minutes).replace("{{name}}", name).replace("{{city}}", city);
    ...
    updateLanguageSelector();
    updatePageContent(lang); // Initial update
    showPopup(); // Initial call
